...
export class LeadershipMessagesRepository {
  ...
  async sent(userId: string, churchId: string) {
    return this.database
      ...
      .leftJoin(
        leadershipMessageRecipients,
        eq(leadershipMessageRecipients.messageId, leadershipMessages.id),
      )
      .where(
        and(
          eq(leadershipMessages.senderUserId, userId),
          eq(leadershipMessages.churchId, churchId),
        ),
      )
  ...
  async create(input: {
    ...
    return this.database.transaction(async (transaction) => {
      ...
      const baseRecipients = transaction
        .selectDistinct({
          userId: users.id,
          phone: users.phone,
        })
      ...
      const recipients =
        input.audience === 'CHURCH'
          ? await baseRecipients.where(
              and(
      ...
      if (recipients.length) {
        await transaction.insert(leadershipMessageRecipients).values(
          recipients.map((recipient) => ({
            messageId: message.id,
            recipientUserId: recipient.userId,
            phoneSnapshot: recipient.phone,
            smsStatus:
              input.smsRequested && recipient.phone
                ? ('QUEUED' as const)
                : ('NOT_REQUESTED' as const),
            smsNextAttemptAt:
              input.smsRequested && recipient.phone ? new Date() : undefined,
          })),
        );
  ...
  async claimSmsBatch(limit = 25) {
    ...
    const candidates = await this.database
      ...
      .innerJoin(
        leadershipMessages,
        eq(leadershipMessages.id, leadershipMessageRecipients.messageId),
      )
      .where(
        and(
          ...
          or(
            isNull(leadershipMessageRecipients.smsAttemptedAt),
            lt(leadershipMessageRecipients.smsAttemptedAt, leaseExpiredAt),
          ),
        ),
      )
    ...
    for (const candidate of candidates) {
      const [row] = await this.database
        .update(leadershipMessageRecipients)
