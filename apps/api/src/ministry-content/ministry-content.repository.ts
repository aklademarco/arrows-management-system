...
export class MinistryContentRepository {
  ...
  async getAccess(userId: string, churchId: string) {
    ...
    const [publicityLeadership, choirLeadership, [mediaDepartment], [choirDepartment]] = await Promise.all([
      this.database
  ...
  async createSongList(
    ...
    return this.database.transaction(async (transaction) => {
      const [content] = await transaction.insert(ministryContent).values({
      ...
      const songs = await transaction.insert(ministrySongItems).values(
      ...
      const recipients = await transaction
        ...
        .where(and(
          inArray(departmentMembers.departmentId, [input.choirDepartmentId, input.mediaDepartmentId]),
          this.activeMembership(today),
          eq(users.accountStatus, 'ACTIVE'),
        ));
      if (recipients.length)
        await transaction.insert(notifications).values(recipients.map(({ userId }) => ({
          churchId: input.churchId,
          recipientUserId: userId,
          actorUserId: input.senderUserId,
          type: 'SONG_LIST',
          title: `New song list: ${input.title}`,
          body: input.instructions || `${songs.length} songs are ready for the service.`,
  ...
  async list(userId: string, churchId: string) {
    ...
    const songContentIds = items.filter((item) => item.type === 'SONG_LIST').map((item) => item.id);
    const songs = songContentIds.length ? await this.database
      .select({
