...
const DAY_OF_TRUMPETS: DefaultItem[] = [
  ...
  {
    title: 'Song Ministration / Project Offering',
    duration: 10,
    owner: 'Choir',
  },
...
export class LiturgiesRepository {
  ...
  async ensureSundayDefaults(churchId: string, actorUserId: string) {
    const definitions = [
      {
        name: 'Normal Sunday Service',
        description:
          'Used every Sunday unless a higher-priority or event-specific service applies.',
        recurrenceRule: 'EVERY_SUNDAY',
        priority: 10,
        isDefault: true,
        items: NORMAL_SERVICE,
      },
      {
        name: 'Day of Trumpets',
        description:
          'Automatically takes priority on the first Sunday of every month.',
        recurrenceRule: 'FIRST_SUNDAY',
        priority: 100,
        isDefault: false,
        items: DAY_OF_TRUMPETS,
      },
    ] as const;
    await this.database.transaction(async (transaction) => {
      const existing = await transaction
        .select({ id: liturgyTemplates.id, name: liturgyTemplates.name })
        .from(liturgyTemplates)
        .where(
          and(
            eq(liturgyTemplates.churchId, churchId),
            inArray(
              liturgyTemplates.name,
      ...
      for (const definition of definitions) {
        ...
        let offset = 0;
        await transaction.insert(liturgyTemplateItems).values(
  ...
  async listTemplates(churchId: string) {
    const templates = await this.database
      ...
      .from(liturgyTemplates)
      .where(eq(liturgyTemplates.churchId, churchId))
      .orderBy(asc(liturgyTemplates.name));
    ...
    const items = templateIds.length
      ? await this.database
          ...
          .from(liturgyTemplateItems)
          .where(inArray(liturgyTemplateItems.templateId, templateIds))
          .orderBy(asc(liturgyTemplateItems.position))
      : [];
  ...
  async eventLiturgy(eventId: string, churchId: string) {
    const [liturgy] = await this.database
      .select({
      ...
      .where(
        and(eq(eventLiturgies.eventId, eventId), eq(events.churchId, churchId)),
      )
      .limit(1);
    ...
    const items = await this.database
      ...
      .where(eq(eventLiturgyItems.liturgyId, liturgy.id))
      .orderBy(asc(eventLiturgyItems.position));
    return { ...liturgy, items };
  }

  async generateEventLiturgy(
    input: GenerateEventLiturgyDto & {
      eventId: string;
      churchId: string;
      actorUserId: string;
    },
    ...
    return this.database.transaction(async (transaction) => {
      ...
      const [existing] = await transaction
        .select({ id: eventLiturgies.id })
        .from(eventLiturgies)
        .where(eq(eventLiturgies.eventId, input.eventId))
        .limit(1);
      if (existing)
        throw new ConflictException('This event already has a liturgy.');
      let template;
      if (input.templateId) {
        [template] = await transaction
          .select()
          .from(liturgyTemplates)
          .where(
            and(
        ...
        const isFirstSunday =
          local.find((part) => part.type === 'weekday')?.value === 'Sun' &&
          Number(local.find((part) => part.type === 'day')?.value) <= 7;
        const candidates = await transaction
          ...
          .where(
            and(
              ...
              eq(liturgyTemplates.isActive, true),
              inArray(
                liturgyTemplates.recurrenceRule,
                isFirstSunday
                  ? ['FIRST_SUNDAY', 'EVERY_SUNDAY']
                  : ['EVERY_SUNDAY'],
              ),
            ),
          );
      ...
      if (!template)
        throw new NotFoundException(
          'No applicable liturgy template was found.',
        );
      const templateItems = await transaction
        .select()
        .from(liturgyTemplateItems)
        .where(eq(liturgyTemplateItems.templateId, template.id))
        .orderBy(asc(liturgyTemplateItems.position));
      const [liturgy] = await transaction
        .insert(eventLiturgies)
        .values({
          eventId: event.id,
        ...
        .values(
          templateItems.map((item) => ({
            ...
            plannedStartAt: new Date(
              event.startsAt.getTime() + item.plannedOffsetMinutes * 60_000,
            ),
  ...
  async controlItem(input: {
    ...
    return this.database.transaction(async (transaction) => {
      const [current] = await transaction
        ...
        .innerJoin(
          eventLiturgies,
          eq(eventLiturgies.id, eventLiturgyItems.liturgyId),
        )
        .innerJoin(events, eq(events.id, eventLiturgies.eventId))
        .where(
          and(
            eq(eventLiturgyItems.id, input.itemId),
            eq(events.churchId, input.churchId),
          ),
        )
        .limit(1)
      ...
      const now = new Date();
      const updates: Partial<typeof eventLiturgyItems.$inferInsert> = {
        timingUpdatedBy: input.actorUserId,
        updatedAt: now,
      };
      let advance = false;
      switch (input.action) {
        case 'START': {
          if (current.status !== 'PENDING')
            throw new ConflictException(
              'Only a pending activity can be started.',
            );
          const [running] = await transaction
            .select({ id: eventLiturgyItems.id })
            .from(eventLiturgyItems)
            .where(
              and(
                eq(eventLiturgyItems.liturgyId, current.liturgyId),
                inArray(eventLiturgyItems.status, ['ACTIVE', 'PAUSED']),
              ),
            )
            .limit(1);
          if (running)
            throw new ConflictException(
              'Complete or skip the current activity first.',
            );
          ...
          await transaction
            .update(eventLiturgies)
            .set({ startedAt: now, updatedAt: now })
            .where(
              and(
                eq(eventLiturgies.id, current.liturgyId),
                isNull(eventLiturgies.startedAt),
              ),
            );
          break;
        }
        case 'PAUSE':
          if (current.status !== 'ACTIVE')
            throw new ConflictException(
              'Only an active activity can be paused.',
            );
        ...
        case 'RESUME':
          if (current.status !== 'PAUSED' || !current.pausedAt)
            throw new ConflictException(
              'Only a paused activity can be resumed.',
            );
          Object.assign(updates, {
            status: 'ACTIVE' as const,
            pausedAt: null,
            accumulatedPauseSeconds:
              current.accumulatedPauseSeconds +
              Math.max(
                0,
        ...
        case 'EXTEND':
          ...
          updates.plannedDurationMinutes =
            current.plannedDurationMinutes + (input.extensionMinutes ?? 0);
          break;
        case 'SKIP':
          if (!['PENDING', 'ACTIVE', 'PAUSED'].includes(current.status))
            throw new ConflictException(
              'This activity can no longer be skipped.',
            );
          Object.assign(updates, {
        ...
        case 'COMPLETE':
          if (!['ACTIVE', 'PAUSED'].includes(current.status))
            throw new ConflictException(
              'Start the activity before completing it.',
            );
          Object.assign(updates, {
      ...
      const [updated] = await transaction
        .update(eventLiturgyItems)
        .set(updates)
        .where(eq(eventLiturgyItems.id, current.id))
        .returning();
      ...
      if (advance) {
        const [next] = await transaction
          ...
          .where(
            and(
              eq(eventLiturgyItems.liturgyId, current.liturgyId),
              gt(eventLiturgyItems.position, current.position),
              eq(eventLiturgyItems.status, 'PENDING'),
            ),
          )
          .orderBy(asc(eventLiturgyItems.position))
          .limit(1)
        ...
        if (next) {
          ...
          await transaction
            ...
            .where(eq(eventLiturgyItems.id, next.id));
        } else {
          await transaction
            .update(eventLiturgies)
            .set({ completedAt: now, updatedAt: now })
            .where(eq(eventLiturgies.id, current.liturgyId));
        }
      }
      await transaction.insert(auditLogs).values({
        churchId: input.churchId,
