...
async function optimizeCover(file: File): Promise<string> {
  ...
  const height = Math.round(
    width * Math.min(bitmap.height / bitmap.width, 0.55),
  );
  ...
  const sourceHeight = Math.min(
    bitmap.height,
    bitmap.width * (canvas.height / canvas.width),
  );
  context.drawImage(
    bitmap,
    0,
  ...
  const save = (value: string) =>
    startTransition(async () => {
      try {
        ...
        const url = await uploadMemberPhoto("cover", value);
        setPreview(url);
