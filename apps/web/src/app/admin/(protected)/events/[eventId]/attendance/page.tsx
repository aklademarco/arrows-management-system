...
export default async function AttendanceRosterPage({
  ...
  const roster = await getAdminResource<Roster>(
    `/attendance/events/${eventId}`,
  );
  const attended = roster.members.filter(
    (member) => member.attendanceId,
  ).length;
