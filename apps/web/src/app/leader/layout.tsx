...
export default async function LeaderLayout({ children }: { children: React.ReactNode }) {
  ...
  return <div className="leader-shell min-h-screen bg-[#f8f7fb] text-slate-950">
    <header className="sticky top-0 z-30 flex items-center justify-between border-b border-purple-100 bg-white/95 px-4 py-3 backdrop-blur lg:hidden">
      <Link className="flex items-center gap-2" href="/leader"><span className="church-logo-tile relative size-10 overflow-hidden rounded-xl bg-white"><Image alt="Arrows church" className="object-contain scale-[1.35]" fill sizes="40px" src="/assets/arrows.PNG" /></span><span className="text-sm font-black">Leadership</span></Link>
    ...
    <div className="mx-auto flex min-h-screen max-w-[1600px]">
      <aside className="sticky top-0 hidden h-screen w-[76px] shrink-0 flex-col items-center bg-gradient-to-b from-[#7e22ce] to-[#5b148d] px-3 py-5 lg:flex">
        <Link className="church-logo-tile relative mb-10 block size-12 overflow-hidden rounded-2xl bg-white shadow-lg" href="/leader"><Image alt="Arrows church" className="object-contain scale-[1.35]" fill sizes="48px" src="/assets/arrows.PNG" /></Link>
        <LeaderNavigation />
