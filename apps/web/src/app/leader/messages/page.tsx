...
type Context = { canMessageChurch: boolean; smsAvailable: boolean; departments: { id: string; name: string }[] };
type SentMessage = { id: string; audience: string; title: string; body: string; sentAt: string; recipientCount: number; smsRequested: boolean; smsQueuedCount: number; smsSentCount: number; smsDeliveredCount: number; smsFailedCount: number };
...
export default async function LeaderMessagesPage() {
  ...
  return <main className="min-h-screen p-4 sm:p-6 xl:p-8"><div className="mx-auto max-w-6xl">
    ...
    <div className="mt-7"><MessageForm canMessageChurch={context.canMessageChurch} departments={context.departments} smsAvailable={context.smsAvailable} /></div>
    <section className="mt-8"><div className="flex items-end justify-between"><div><p className="text-xs font-black uppercase tracking-wider text-purple-300">Delivery history</p><h2 className="mt-1 text-2xl font-black">Sent messages</h2></div><span className="text-sm font-bold text-slate-500">{sent.length}</span></div>
      {sent.length ? <div className="mt-4 space-y-3">{sent.map((message) => <article className="rounded-[1.5rem] border border-white/10 bg-[#24202e] p-5" key={message.id}><div className="flex gap-4"><span className="grid size-11 shrink-0 place-items-center rounded-xl bg-purple-500/15 text-purple-300"><FiMessageCircle /></span><div className="min-w-0 flex-1"><div className="flex flex-wrap justify-between gap-2"><h3 className="font-black">{message.title}</h3><time className="text-xs font-bold text-slate-500">{formatter.format(new Date(message.sentAt))}</time></div><p className="mt-2 line-clamp-3 whitespace-pre-wrap text-sm leading-6 text-slate-400">{message.body}</p><p className="mt-3 flex items-center gap-2 text-xs font-bold text-purple-300"><FiUsers /> {message.recipientCount} recipient{message.recipientCount === 1 ? "" : "s"} · {message.audience === "CHURCH" ? "Whole church" : "Department"}</p>{message.smsRequested ? <div className="mt-3 flex flex-wrap gap-2 text-[10px] font-black uppercase tracking-wide"><span className="rounded-full bg-amber-300/10 px-2.5 py-1 text-amber-200">{message.smsQueuedCount} queued</span><span className="rounded-full bg-blue-300/10 px-2.5 py-1 text-blue-200">{message.smsSentCount} sent</span><span className="rounded-full bg-emerald-300/10 px-2.5 py-1 text-emerald-200">{message.smsDeliveredCount} delivered</span>{message.smsFailedCount ? <span className="rounded-full bg-red-300/10 px-2.5 py-1 text-red-200">{message.smsFailedCount} failed</span> : null}</div> : null}</div></div></article>)}</div> : <div className="mt-4 grid min-h-52 place-items-center rounded-[1.5rem] border border-dashed border-white/15 bg-[#24202e]"><div className="text-center"><FiMessageCircle className="mx-auto text-4xl text-purple-400" /><p className="mt-3 font-black">No messages sent yet</p></div></div>}
    </section>
  </div></main>;
}
