          ...
          <div className="px-5 py-7 sm:px-8">
            <div className="flex flex-wrap items-end justify-between gap-4">
              <ProfileAvatar
                ...
                variant="admin"
              />
              <span className="mb-1 inline-flex items-center gap-2 rounded-full bg-purple-500/15 px-4 py-2 text-xs font-black text-purple-200">
                <FiShield /> {role}
              </span>
            </div>
            <h2 className="mt-4 text-2xl font-black">{name}</h2>
            {profile.directoryBio ? (
        ...
        <div className="mt-5 grid gap-5 lg:grid-cols-2">
          ...
          <section className="rounded-[1.5rem] border border-white/10 bg-[#24202e] p-5">
            <h2 className="flex items-center gap-2 font-black">
              <FiUsers className="text-purple-300" /> Ministry responsibility
            </h2>
            <div className="mt-4 flex flex-wrap gap-2">
              {account.memberProfile?.primaryDepartment ? (
                <span className="rounded-full bg-purple-500/15 px-3 py-1.5 text-xs font-bold text-purple-200">
                  {account.memberProfile.primaryDepartment.name}
                </span>
              ) : (
                <p className="text-sm text-slate-400">Church-wide leadership</p>
              )}
            </div>
          </section>
        </div>

        <section className="mt-5 rounded-[1.5rem] border border-white/10 bg-[#24202e] py-3">
          <ThemeSetting />
        </section>
          ...
          <section className="mt-5 rounded-[1.5rem] border border-white/10 bg-[#24202e] p-5">
            ...
            <div className="mt-4 flex flex-wrap gap-2">
              {profile.skills.map((skill) => (
                <span
                  className="rounded-full bg-lime-300/10 px-3 py-1.5 text-xs font-bold text-lime-300"
                  key={skill}
                >
                  {skill}
                </span>
